...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-white">Edit Country</h1>
            <p class="text-gray-400 mt-1">Update country information</p>
        </div>
        ...
        <form action="{{ route('admin.countries.update', $country->id) }}" method="POST"
            ...
            <div>
                ...
                <input type="number" name="student_count" id="student_count"
                    value="{{ old('student_count', $country->student_count) }}" min="0"
                    class="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="6"
                    class="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="Enter country description...">{{ old('description', $country->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
