...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-white">Scholarships Management</h1>
            <p class="text-gray-400 mt-1">Manage scholarships and funding opportunities</p>
        </div>
        ...
        @foreach($scholarships as $scholarship)
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-6 hover:bg-gray-750 transition-colors">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex-1">
                        <h3 class="text-xl font-bold text-white mb-1">{{ $scholarship->name }}</h3>
                        <p class="text-gray-400 text-sm">Scholarship ID: {{ $scholarship->id }}</p>
                    </div>
                ...
                <div class="space-y-3">
                    @if($scholarship->amount)
                        <div class="flex items-center text-green-400">
                            ...
                            <span class="text-sm font-semibold">${{ number_format($scholarship->amount) }}</span>
                        </div>
                    @endif

                    <div class="text-gray-300 text-sm line-clamp-3">
                        {{ Str::limit($scholarship->description, 150) }}</div>

                    <div class="flex items-center justify-between pt-3 border-t border-gray-700">
                        <span class="text-xs text-gray-500">ID: {{ $scholarship->id }}</span>
    ...
    @if($scholarships->isEmpty())
        <div class="text-center py-12">
            ...
            <h3 class="text-xl font-semibold text-gray-400 mb-2">No scholarships found</h3>
            <p class="text-gray-500 mb-6">Get started by adding your first scholarship.</p>
            <a href="{{ route('admin.scholarships.create') }}"
