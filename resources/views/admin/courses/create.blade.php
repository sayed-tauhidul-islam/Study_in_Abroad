...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-white">Add New Course</h1>
            <p class="text-gray-400 mt-1">Create a new course for a university</p>
        </div>
        ...
        <form action="{{ route('admin.courses.store') }}" method="POST" class="space-y-6">
                ...
                <select name="university_id" id="university_id"
                    ...
                    @foreach($universities as $university)
                        <option value="{{ $university->id }}"
                            {{ (old('university_id') ?? request('university_id')) == $university->id ? 'selected' : '' }}>
                            {{ $university->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <div>
                    ...
                    <select name="level" id="level"
                        ...
                        <option value="Associate"
                            {{ old('level') == 'Associate' ? 'selected' : '' }}>
                            Associate</option>
                        <option value="Bachelor"
                            {{ old('level') == 'Bachelor' ? 'selected' : '' }}>
                            Bachelor</option>
                        <option value="Master"
                            {{ old('level') == 'Master' ? 'selected' : '' }}>
                            Master</option>
                        <option value="Doctor"
                            {{ old('level') == 'Doctor' ? 'selected' : '' }}>
                            Doctor (PhD)</option>
                    </select>
                    @error('level')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="tuition_fee" id="tuition_fee"
                        value="{{ old('tuition_fee') }}" step="0.01" min="0"
                ...
                <div>
                    ...
                    <input type="number" name="application_fee" id="application_fee"
                        value="{{ old('application_fee') }}" step="0.01" min="0"
            ...
            <div>
                ...
                <textarea name="requirements" id="requirements" rows="4"
                    ...
                    placeholder="High school diploma, Mathematics proficiency, English language test (TOEFL/IELTS)">{{ old('requirements') }}</textarea>
                @error('requirements')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
