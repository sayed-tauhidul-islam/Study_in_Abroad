...
<div class="bg-white rounded-lg shadow-md p-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Create New Degree</h1>
        <p class="text-gray-600 mt-2">Add a new academic degree to the system.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.degrees.store') }}">
        ...
        <div class="mt-6">
            ...
            <textarea name="description" id="description" rows="4"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Describe the degree program...">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
